    ...
    return (
        <div className="flex-1 overflow-y-auto mt-4 space-y-4 pb-4">
            {filteredData.map((item) => (
                <div
                    key={item.requestId}
                    onClick={() => router.push(`/home/${item.requestId}`)} // Handle click to navigate to the dynamic route
                    className="flex flex-col sm:flex-row items-start sm:items-center bg-white p-4 sm:p-5 rounded-2xl shadow-md border border-neutral-200 hover:shadow-lg transition-shadow duration-200 cursor-pointer"
                >
                    {/* Content */}
